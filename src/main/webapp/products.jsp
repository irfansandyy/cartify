...
<main class="container mx-auto px-6 py-8 space-y-4">
    ...
    <c:if test="${empty products}">
        <div class="card shadow-md rounded-xl text-center py-10 space-y-3">
            ...
            <a class="btn btn-accent rounded-full px-6 py-2 text-sm inline-flex items-center justify-center gap-2 hover:-translate-y-0.5 transition-transform" href="${pageContext.request.contextPath}/seed">Run Seeder</a>
        </div>
    </c:if>
            ...
            <article class="card shadow-sm rounded-xl overflow-hidden flex flex-col hover:-translate-y-1 hover:shadow-md transition-transform duration-150">
                <a href="${pageContext.request.contextPath}/product?id=${p.id}" class="flex flex-col flex-1">
                    ...
                    <div class="p-4 flex flex-col gap-2">
                        <div class="flex items-center justify-between mb-1">
                            <div class="flex flex-wrap gap-1">
                                <c:if test="${not empty p.categoryId}">
                                    <span class="inline-flex items-center rounded-full bg-accent px-2 py-0.5 text-[10px] font-medium text-muted uppercase tracking-wide">Category ${p.categoryId}</span>
                                </c:if>
                            </div>
                        ...
                        <p class="text-muted text-xs leading-snug">${fn:length(p.description) > 80 ? fn:substring(p.description,0,77) + '...' : p.description}</p>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <form method="post" action="${pageContext.request.contextPath}/cart" class="m-0">
                                ...
                                <button class="inline-flex items-center justify-center gap-2 rounded-lg w-full text-xs bg-primary text-white px-4 py-2 transition-transform hover:-translate-y-0.5 hover:bg-primary-hover" type="submit">
                                    <img src="${pageContext.request.contextPath}/assets/icons/shopping-cart.svg" alt="Add to cart" class="h-3.5 w-3.5" />
